...
bsc::dsp::VoiceConfig MartigliNode::buildSharedConfig() const {
...
void MartigliNode::loadSharedEngine(float initialElapsed) {
...
void MartigliNode::syncSharedConfig() {
    if (_sharedEngineLoaded) {
        _sharedEngine.updateVoice(0, buildSharedConfig());
    }
}

void MartigliNode::publishSharedSnapshot(double snapshotAudioTime) {
...
void MartigliNode::processNode(const std::shared_ptr<AudioBus> &bus, int framesToProcess) {
    ...
    const double blockStartTime = _context ? _context->getCurrentTime() : 0.0;
    ...
    for (int i = 0; i < framesToProcess; ++i) {
        ...
        if (leftChannel) leftChannel[i] = (numChannels >= 2 ? sampleL : sampleL + sampleR) * _smoothedVolume;
